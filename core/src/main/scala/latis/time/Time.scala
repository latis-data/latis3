...
import latis.data
import latis.data.{BinaryData, Data, SampledFunction, Text}
...
class Time(metadata: Metadata) extends Scalar(metadata) {
  ...
  val timeFormat: Option[TimeFormat] = this("type") match {
    case Some("string") => this("units") match {
      case Some(f) => 
   ...
   */
  override def convertValue(value: String): Data = timeFormat map { format =>
    Data(format.format(TimeFormat.parseIso(value))) //TODO: error if not ISO
  } getOrElse {
    convertValue(value)  //treat like any other value
  }
