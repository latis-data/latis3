...
//  }
  ...
  def applyToData(ds1: Dataset, ds2: Dataset): SampledFunction = {
    ...
    def f(sample: Sample, path: SamplePath): Sample = path match {
       ...
       */
      case (head :: tail) if (tail.isEmpty) => head match {
        case DomainPosition(i) =>
          ...
          val domain = DomainData.fromSeq(vals2)
        ...
        case RangePosition(i) =>
          ...
          val range = RangeData.fromSeq(vals)
  ...
  def applyToModel(dt1: DataType, dt2: DataType): DataType = {
    ...
    def go(dt: DataType): DataType = dt match {
      ...
      case tup @ Tuple(es @ _*) => es.map(_.id).indexOfSlice(vids) match {
        case -1 => Tuple(es.map(go): _*) //no match, keep recursing
        case index => es.splitAt(index) match {
          // Splice in the new variable types
          case (p1, p3) => 
            val dts = (p1 :+ range ++ p3.drop(vids.length))
            if (dts.length == 1) dts.head  //Reduce 1-Tuple //TODO: Tuple constructor option? Tuple.reduced()? vs flattened
            else Tuple(dts: _*)
        }
      }
