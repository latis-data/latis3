...
case class GroupByVariables(variableNames: String*)
  (val aggregation: Aggregation = NoAggregation()) 
  ...
  def groupByFunction(model: DataType): Sample => Option[DomainData] = {
    val paths = variableNames.flatMap(model.getPath(_))  //TODO: deal with invalid name
