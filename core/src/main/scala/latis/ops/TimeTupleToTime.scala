...
case class TimeTupleToTime(id: Identifier = id"time") extends MapOperation {
  def applyToModel(model: DataType): Either[LatisException, DataType] =
    Either.catchOnly[LatisException](model.map {
      case t @ Tuple(es @ _*) if t.id.contains(id) => //TODO: support aliases with hasName?
        ...
        val format: String = es.toList
          .traverse(_("units"))
          .fold(throw LatisException("A time Tuple must have units defined for each element."))(
            _.mkString(" ")
          )
  ...
  override def mapFunction(model: DataType): Sample => Sample = {
    ...
    val timeLen: Int = model.findVariable(id).getOrElse {
      throw new LatisException(s"Cannot find variable: ${id.asString}")
    } match {
      case t: Tuple =>
        t.flatten match {
          case tf: Tuple =>
            tf.elements.length //TODO: is this "dimensionality"? Should it be a first class citizen?
...
object TimeTupleToTime {
  def fromArgs(args: List[String]): Either[LatisException, TimeTupleToTime] =
