...
final class FdmlParserSpec extends AnyFlatSpec {
  "An FDML parser" should "parse a valid FDML file" in
    withFdmlFile("fdml-parser/valid.fdml") { fdml =>
      inside(fdml.right.value) {
        case DatasetFdml(metadata, source, adapter, model, operations) =>
          ...
          inside(model) {
            case FFunction(domain, range, attrs) =>
              attrs should contain("id" -> "outer")

              inside(domain) {
                case FScalar(id, ty, attrs) =>
              ...
              inside(range) {
                case FFunction(domain, range, attrs) =>
                  ...
                  inside(domain) {
                    case FTuple(fst, snd, rest, attrs) =>
          ...
          inside(operations) {
            case firstOp :: secondOp :: thirdOp :: Nil =>
  ...
  it should "parse valid granule append FDML" in
    withFdmlFile("fdml-parser/granule-append.fdml") {
      ...
      case Right(fdml) =>
        inside(fdml) {
          case GranuleAppendFdml(metadata, source, adapter, model, ops) =>
            metadata.properties should contain("id" -> "granuleAppend")

            inside(source) {
              case FdmlSource(granuleFdml) =>
                inside(granuleFdml) {
                  case DatasetFdml(metadata, source, adapter, model, _) =>
                    inside(source) {
                      case UriSource(uri) =>
                    ...
                    inside(adapter) {
                      case SingleAdapter(clss, _) =>
                        clss should equal("granule-class")
                    }

                    inside(model) {
                      case FFunction(domain, range, _) =>
                        inside(domain) {
                          case FScalar(id, ty, attrs) =>
                            id should equal(id"time")
                            ty should equal(IntValueType)
            ...
            inside(adapter) {
              case NestedAdapter(clss, _, nested) =>
