...
class CurrySuite extends AnyFunSuite {
  test("Curry a 2D dataset") {
    ...
    inside(samples.head) {
      case Sample(DomainData(Number(t)), RangeData(f)) =>
        ...
        inside(f) {
          case MemoizedFunction(samples) =>
            inside(samples) {
              case Vector(s1, s2) =>
                inside(s1) {
                  case Sample(DomainData(Text(d)), RangeData(Number(r1), Number(r2))) =>
                    assert(d == "Fe")
                    assert(r1 == 1.1)
                    assert(r2 == 0.1)
                }
  ...
  test("Curry a 2D dataset to arity 2 (no change)") {
    ...
    inside(samples.head) {
      case Sample(DomainData(Number(d1), Text(d2)), RangeData(Number(r1), Number(r2))) =>
        assert(d1 == 1)
        assert(d2 == "Fe")
        assert(r1 == 1.1)
        assert(r2 == 0.1)
        ...
        inside(f) {
          case MemoizedFunction(samples) =>
            inside(samples) {
              case Vector(s1, s2) =>
                inside(s1) {
                  case Sample(DomainData(Integer(d)), RangeData(Number(r))) =>
                    assert(d == 1)
                    assert(r == 10.0)
                }
