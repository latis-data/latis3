...
case class DetectAnomaliesWithAutoencoder(
  ...
  override def applyToData(data: SampledFunction, model: DataType): SampledFunction = {
    ...
    try {
      ...
      interp.exec(s"ts_with_model = autoencoder_prediction(dataset, '$dsName', $trainSize, var_name='$varName')")
      ...
      val samplesWithAnomalyData: Seq[Sample] = samples.zipWithIndex.map { case (smp, i) =>
        smp match {
          case Sample(dd, rd) => {
             Sample(dd, rd :+ Data.DoubleValue(modelOutputCol(i)) :+ Data.BooleanValue(anomalyCol(i))) 
          }
        }
      }
