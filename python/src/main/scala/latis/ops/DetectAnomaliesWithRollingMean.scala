 ...
 * TODO: for each argument the Python algorithms take, add an argument here?
 */
case class DetectAnomaliesWithRollingMean() extends UnaryOperation {
  ...
  override def applyToData(data: SampledFunction, model: DataType): SampledFunction = {
    ...
    try {
      ...
      interp.exec(s"ts_with_model = model_with_rolling_mean(ds_numpy, 10, '$ds_name', var_name='$var_name')")
      interp.exec(s"X = ts_with_model['$var_name']")
      interp.exec(s"Y = ts_with_model['$alg_name']")
      interp.exec(s"ts_with_anomalies = detect_anomalies(X, Y, '$ds_name', '$var_name', '$alg_name', outlier_def='errors', num_stds=2)")
