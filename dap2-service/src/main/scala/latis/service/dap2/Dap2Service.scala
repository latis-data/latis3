...
class Dap2Service(catalog: Catalog) extends ServiceInterface(catalog) with Http4sDsl[IO] {
  override def routes: HttpRoutes[IO] =
    HttpRoutes.of {
      case req @ GET -> Root / id ~ ext =>
        (for {
          ident <- IO.fromOption(Identifier.fromString(id))(
            ParseFailure(s"'$id' is not a valid identifier")
          )
      ...
      .flatMap { cexprs: ConstraintExpression =>
        cexprs.exprs.traverse {
          ...
          case ast.Operation(name, args) =>
            UnaryOperation
              .makeOperation(name, args.map(stripQuotes(_)))
  ...
  private def encode(
    ext: String,
    ds: Dataset
  ): Either[Dap2Error, (Stream[IO, Byte], `Content-Type`)] = ext match {
    case "" => encode("html", ds)
    case "bin" =>
      new BinaryEncoder()
        .encode(ds)
        .flatMap { bits =>
          Stream.emits(bits.toByteArray)
        }
        .asRight
        .map((_, `Content-Type`(MediaType.application.`octet-stream`)))
    case "csv" =>
      CsvEncoder.withColumnName
        .encode(ds)
